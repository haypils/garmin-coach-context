...
def _classify_sport(sport_type: str) -> str:
    """Classify sport type into categories."""
    s = sport_type.lower()
...
def _build_weekly_summaries(activities: list[Activity]) -> list[WeeklySummary]:
    ...
    for a in activities:
        ...
        sport = _classify_sport(a.sport_type or a.activity_type)
        if sport == "swim":
            w.swim_hours += hours
            w.swim_km += km
            w.swim_sessions += 1
...
def build_context(output_path: Path | None = None, use_db: bool = True) -> Path:
    ...
    if use_db:
        ...
        weeks = _build_weekly_summaries(activities)
